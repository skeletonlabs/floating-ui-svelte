...
type MaybeBoxOrGetter<T> = T | Getter<T> | ReadableBox<T>;
...
type WritableBox<T> = ReadableBox<T> & {
 ...
 * @see {@link https://runed.dev/docs/functions/box}
 */
function box<T>(): WritableBox<T | undefined>;
/**
 ...
 * @see {@link https://runed.dev/docs/functions/box}
 */
function box<T>(initialValue: T): WritableBox<T>;
function box(initialValue?: unknown) {
 ...
 * @see {@link https://runed.dev/docs/functions/box}
 */
function boxFlatten<R extends Record<string, unknown>>(
	boxes: R,
): BoxFlatten<R> {
	return Object.entries(boxes).reduce<BoxFlatten<R>>(
		(acc, [key, b]) => {
			if (!box.isBox(b)) {
