...
import type { HandleCloseFn } from '$lib/hooks/useHover/index.svelte.js';
...
export function safePolygon(options: SafePolygonOptions = {}) {
	const { buffer = 0.5, blockPointerEvents = false, requireIntent = true } = $derived(options);
	...
	const fn: HandleCloseFn = ({ x, y, placement, elements, onClose /* nodeId, tree */ }) => {
		return function onMouseMove(event: MouseEvent) {
			...
			const isOverReferenceEl = contains(elements.reference, target);
